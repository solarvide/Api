...
namespace GoogleAuthentication {
    public class GAuthenticator {
        ...
        private string GetQRCodeUrl() {
            ...
            var base32Secret = Base32.Encode(Secret);
            var url = string.Format("http://chart.apis.google.com/chart?chs={0}x{0}&cht=qr&chl=otpauth://totp/{1}%3Fsecret%3D{2}{3}", QRCodeSize, Identity, base32Secret, (string.IsNullOrEmpty(Issuer) ? string.Empty : string.Format("%26issuer%3D{0}", Issuer)));
